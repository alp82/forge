...
def test_plugin_json_version_1_2_18():
    """plugin.json version must be '1.3.2'."""
    plugin_path = Path(__file__).resolve().parents[2] / ".claude-plugin" / "plugin.json"
    data = json.loads(plugin_path.read_text(encoding="utf-8"))
    assert (
        data["version"] == "1.3.2"
    ), f"plugin.json version must be '1.3.2', got {data['version']!r}"


# --- RC4-E02 ---
def test_marketplace_json_version_1_2_18_matches_plugin():
    """marketplace.json version must be '1.3.2' and equal plugin.json."""
    ...
    assert (
        market_ver == "1.3.2"
    ), f"marketplace.json version must be '1.3.2', got {market_ver!r}"
    assert (
