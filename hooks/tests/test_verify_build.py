...
branch) CLEARS the change marker. A live, non-converged, cwd-matching,
...
def test_vb_g13_live_run_exemption_string_route_applies_identically():
    """TC-VB-G13: same fixture, route a non-empty STRING -> exemption applies
    identically."""
    assert shutil.which(
        "npm"
    ), "npm required to drive build output through the jq reason path"
...
def test_vb_g14_live_run_exemption_pending_gate_alone_is_non_converged():
    """TC-VB-G14: same fixture, route=[] and pending_gate="plan-approval" ->
