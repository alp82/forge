"""Failing (red) tests for hooks/mark-code-change.py.
...
CONTRACT:
  ...
  - Arms two session-keyed marker files when, and only when, ALL of:
...
(_run_hook, lines 36-49). No inline shell echoes of git verbs in any payload here.
"""
...
def test_edit_under_cwd_arms_both_markers_silently():
    ...
    try:
        ...
        _cleanup(tests_marker, build_marker)
...
def test_missing_session_id_falls_back_to_pid_keyed_markers():
    before = set(glob.glob("/tmp/.claude-code-changed-tests-fallback-*")) | set(
        glob.glob("/tmp/.claude-code-changed-build-fallback-*")
    )
    cwd = tempfile.mkdtemp()
    new_markers = []
    try:
        ...
        assert result.stdout.strip() == "", f"got {result.stdout!r}"
        after = set(glob.glob("/tmp/.claude-code-changed-tests-fallback-*")) | set(
            glob.glob("/tmp/.claude-code-changed-build-fallback-*")
        )
        new_markers = sorted(after - before)
        assert len(new_markers) == 2, (
            "expected exactly one new fallback tests marker and one new fallback "
            f"build marker to appear; new_markers={new_markers!r}"
        )
    finally:
        shutil.rmtree(cwd, ignore_errors=True)
