...
if tail -40 "$transcript_path" 2>/dev/null | grep -qF '<!-- pipeline-complete -->'; then
  cat <<'JSON'
{"hookSpecificOutput": {"hookEventName": "UserPromptSubmit", "additionalContext": "FOLLOW-UP REMINDER: a pipeline completed in this conversation. This new request is a NEW TASK - run the pipeline in WORKFLOW.md ## Pipeline directly (intent handling per ### Intent). When the request is clear, state the one-line interpretation inline and proceed immediately - no confirmation gate; the user corrects in their next message if the reading is wrong. Only when the ask is genuinely ambiguous, restate the outcome in 1-2 sentences and WAIT: on an affirmation (y/yes/correct/proceed/looks right) continue, and on any non-affirmation reply (free text, additions, corrections) treat it as reshape and spawn the clarifier with that reply as the new RAW_REQUEST. The pipeline self-adapts to whatever tier the classifier returns - no command dispatch needed; the main agent orchestrates from WORKFLOW.md."}}
JSON
else
  cat <<'JSON'
