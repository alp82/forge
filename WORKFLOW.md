...
2. **Render.** Show the route as **native markdown** - emit it directly from state, rendered (not fenced), no script, no Bash. The render card IS the narration for the route/recompose; there is no separate inline play-by-play. Render **every turn** so progress is never invisible: the full route on the first turn and at any gate (legibility A), the delta on a plain recompose (legibility B). This is a status surface, not a question - it never interrupts; asking the user is what a gate stage does when it runs (step 3, see `## Gates`). Formats:
   - **Full (A)** - a header `path · size · N stages`, then the route as a phase-grouped nested list - grammar, markers, and phase order per `doctrine/render-card.md`. This step just says when to render. The markers always render natively. The router returns `waves` (the topo levels); stages in one wave share no precedence edge and dispatch as a single parallel batch, each marked `▶`. The render composes `route` + `held` so a gated stage stays visible. At a plan-approval gate the card also carries the producer's Plan Breakdown as a short **Plan breakdown** section beneath the list. Example, emitted exactly as this native markdown (no fence):
     ...
     *code · M · 5 stages*
     ...
     - 📐 **Blueprint**
       - • code-planner
     - 🔨 **Build**
       - 🔒 code-implementer (waiting on validated tests and an approved plan)
     - 🔬 **Review**
...
- **`sketch`** - sandboxed throwaway (`.prototypes/`) in any medium: a code tracer-bullet (`sketch-build`), a diagram, a UI mockup, an idea sketch. Runs relaxed; the code-only ceremony band (challenge, Document, the shape/conventions/acceptance lenses) is filtered off the path by each stage's `routes`. Correctness and security still apply. Graduating flips to `code`/`system`.
- **`code`** - the full composed route. A bug is a code build: `triage` pairs `code` with a `bug` signal, the `code-investigator` diagnoses inside the route, and the code path fixes the cause. There is no separate `diagnose` path. Two independent axes shape a code build: `#needs-tests` (the TDD axis - a change carrying real logic) and `#significant-build` (the review-depth axis - a serious build that pulls Scout and the deep lenses). The `code-implementer` carries two locks (see `## Locks`): the TDD gate `{while:#needs-tests, until:#tests-ready}` - on a logic change held until `test-review` publishes `#tests-ready`, inactive on a trivial change - and the plan gate `{while:#plan-ready, until:#plan-approved}`, which is armed on every planned code build until the plan is approved and inactive on the `#direct-impl` short path (no `#plan-ready` live -> gate inactive).
...
- **code** - `{"live":["code","needs-tests","significant-build","plan-ready","plan-approved","code-written","ui-touched","auth-surface","perf-surface"],"available":["confirmed-intent","diff"]}` composes `reuse-scanner` + `health-checker` + `prototype-identifier` (Scout, pulled by `#significant-build`), the full Review wave (correctness via `#code-written`; simplicity via `#plan-ready` (planned builds); shape, conventions, acceptance via `#significant-build`; performance via `#perf-surface`; test-gap, test-verifier via `#needs-tests`; ux, accessibility, design-consistency via `#ui-touched`), `security-reviewer` pulled in by `auth-surface`, and `capture-agent`. The two axes are independent: `#needs-tests` pulls only the TDD chain, `#significant-build` pulls Scout and the deep lenses. The lenses share `@diff`, so they land in one parallel wave. Size XXL.
- **trivial code** - `{"live":["code","direct-impl"],"available":["confirmed-intent"]}` composes exactly `code-implementer` (size XS) - the short path skips the planner entirely. Both of the implementer's locks are inactive: no `#needs-tests` (trivial change, TDD gate inactive) and no `#plan-ready` (plan gate inactive by silence), so the implementer runs straight off `@confirmed-intent` with no plan and is neither held nor waiting on `#plan-approved`. Once `#code-written` makes `@diff` available, `correctness-reviewer` joins; `simplicity-reviewer` does NOT (it subscribes `#plan-ready`, which the short path never publishes). None of the deep lenses, Scout, clarify, test-chain, challenge, or Document join.
...
The **plan-approval gate** holds both implementers behind `#plan-approved` (see `## Locks`). On the `code` path with `#significant-build` live, `plan-challenger` runs in-route and its Approve publishes `#plan-approved`; on a multi-plan run (`doctrine/multi-plan.md`) the per-plan challengers run critique-only and `plan-arbiter` is the in-route publisher instead, on its Adopt verdict. On the system and small planned (logic) build paths there is no in-route publisher, so the orchestrator emits `#plan-approved` itself: a one-tap confirm before execution (system: always; small planned build: only when the plan touches `>1` file or `est-size > S`, else auto-published). The `#direct-impl` short path has no plan gate at all - no `#plan-ready` is ever live on it, so nothing holds the implementer and no `#plan-approved` is owed. The system confirm asks "Run this system change?" with header `Run change` and options Proceed / Abort; the small planned-build confirm asks "Proceed with this change?" with header `Proceed` and options Proceed / Hold, where the Hold option's description carries the escalation - declining escalates to the full challenge. Each one-tap confirm renders the planner's Plan Breakdown in its card, and the gate's concrete example rides in that card breakdown, not in the option descriptions (which stay fixed-process example-exempt). (The auto/confirm threshold stated here - `>1 file or est-size > S` - is the De Morgan dual of the `## Locks` release policy - `<=1 file AND est-size <= S` - see that section for the absent-est-size default.)
...
Each stage declares its own `model` in agent frontmatter. `fable` (Mythos-class, the
top tier) is for the stages carrying the deepest reasoning load - generative planning
and design, adversarial judgment, and intent extraction, plus the deep-reasoning build,
investigation, and authoring stages (the planners, the challenger and arbiter, the deep
review lenses, the intent stages, the generative prototypers, the implementer, the code
investigator, the TDD validation gate, the talk-path specialist, and the capture/ADR/setup
authors); `sonnet` for the stages that execute
...
gate, the system one-tap confirm, the small-planned-build one-tap confirm - the orchestrator
...
A hard session kill - not just compaction - is recoverable because the orchestrator persists its canonical run state to disk every turn. The snapshot lives at the per-run path `.alp-river/runs/<run-id>/run-state.json`, where `<run-id>` is the session id, so two pipelines in one repo never clobber one file. The orchestrator writes the snapshot with its own Write tool as the HARD REQUIRED step-4 action (loop step 4 owns the write contract and the exact schema; this section does not restate them). The SessionStart recovery hook `hooks/recover-run-state.sh` (registered on startup, resume, and compact) reads it back and always injects the per-run write path so the orchestrator learns where to persist. A companion SessionStart hook, `hooks/ensure-gitignore.sh` (startup and resume), keeps this directory out of version control - it adds `.alp-river/` to the repo's `.gitignore` when absent, and if the directory was already committed it injects a turn-1 instruction to surface an untrack command for the user (git rm is user-only, per the git-write guard).

**Read-back guards.** A candidate run-state.json is offered only when it passes every guard: **G1** validate-on-read - `jq`-parseable AND `schema_version==1` AND the required fields present (a malformed or wrong-schema file is rejected, never injected as garbage); **G2** its `cwd` equals the session cwd; **G4** its file mtime is within `RIVER_STATE_MAX_AGE_SECONDS` (default 86400) of now. Freshness is the file's OS mtime - there is no `written_at` field. Selection is per-source: resume reads ONLY its own `runs/<session_id>/` dir, while startup SCANS `runs/*/` and picks the freshest passing candidate (a brand-new startup session_id matches no dir, so recovery is by cwd + age + freshest-scan, never session-equality). A **converged** state (empty route AND empty/absent pending gate) is never offered, and dirs older than the max-age are **pruned** during the read pass. When a candidate passes, the hook injects a recovery offer naming its mid-run stage and route as a turn-1 instruction to surface a confirm-first gate ("resume at <stage> on route <route>, or start fresh?"); since SessionStart additionalContext injects SILENTLY (no picker can render), the offer is best-effort - if it is ignored or compacted away it is a silent no-op.

**Session-id precondition.** Resume reads only its own `runs/<session_id>/` dir; this assumes the harness preserves the session_id across a hard-kill -> resume cycle. If a new id is minted instead, recovery falls back to the next genuine startup scan (safe degradation - no data is lost, the next startup picks up the freshest passing candidate by cwd + age).

**Honest scoping.** Durability rests on the hard per-turn write plus `already_run`/`live`/`available` accuracy and reconcile-from-tree (`## Recovery`), NOT on G4 catching a missed write: a missed write leaves a recent-but-stale file G4 would pass, so route-recomputation plus reconcile-from-tree is the real backstop. The live background-task handle stays non-recoverable - reconcile the mid-run stage from the working tree per `## Recovery`. A malformed write degrades to anchor-only with the preserve-manually note (terseness in the free-text fields is the mitigation). A pipeline started right after a mid-session `/clear` is non-durable until the next genuine session start - the recovery hook deliberately skips `clear`. Only a pending plan-approval gate is re-emitted (the hook carries its plan handle path from the artifact index); any other mid-flight picker simply re-runs its read-only stage.
...
Both implementers also carry a second `{while:#plan-ready, until:#plan-approved}` lock - the **plan-approval gate**. `#plan-ready` fires the moment a plan exists, so this lock is armed on every planned code or system build; the implementer/executor is held until `#plan-approved` clears it. No planned code or system change ever starts against an unapproved plan. The `#direct-impl` short path has no plan gate at all: no `#plan-ready` is ever live on it, so the lock is inactive by silence and the implementer runs straight off `@confirmed-intent`.

**Release policy.** Unlike the safety gate - a real `routes:[system,code]` stage (`safety-gate`) that publishes `#safety-approved` in-route - `#plan-approved` has **no in-route publisher** on the system or small planned-build path: `plan-challenger` is `routes:[code]` and only runs on a `#significant-build`. So on those paths the orchestrator emits `#plan-approved` as a **HARD REQUIRED step**. The cue is a stage sitting in `held` keyed on `#plan-approved` with no in-route producer. The `#direct-impl` short path never reaches this policy - it carries no plan gate, so nothing sits held on `#plan-approved`. (The threshold below is the De Morgan dual of the `## Gates` plan-approval paragraph - `>1 file or est-size > S` - kept in sync by this cross-reference.)
...
- **Small planned build** - auto-publish `#plan-approved` iff the plan touches `<=1` file AND `est-size <= S`; otherwise a one-tap confirm asking "Proceed with this change?", header `Proceed`, options Proceed / Hold (the Hold description states that declining escalates to the full challenge). When `#est-size` is not live (triage may omit it on some paths), treat the plan as NOT small - apply a one-tap confirm, never auto-publish. Declining the one-tap escalates into the full `plan-challenger` band (publish `#significant-build`, run the challenger, which then owns the `#plan-approved` release). `est-size` stays advisory for stage selection but is **load-bearing** for this auto-release decision only - when absent, the safe default is confirm.
...
- **Option, not a signal.** Both `See it in plain words` (the inline plain re-render that rides the picker on the four dense pickers) and `See it as an interactive doc` (the `.briefs/` HTML page, pulled from inside that plain re-render) are handled the same orchestrator-inline way as the small planned-build Hold: read off the pick, never a routing signal (same pattern as the small planned-build Hold escalation above).
...
**HARD withhold (HARD REQUIRED, same emphasis as the `#plan-approved` orchestrator-emit).** Until `#milestones-complete`, the orchestrator MUST NOT add `@diff` to `available` and MUST NOT publish `#code-written`. This keeps the End Review wave (the global lenses over the whole `@diff`) from firing per milestone; it fires exactly once, after the final milestone, when `@diff` becomes available and `#code-written` publishes. `#milestones-complete` is orchestrator-emitted, like the other orchestrator-sourced control signals. No git commits happen between milestones - the working tree accumulates across all milestones and only the final cumulative diff is the artifact the End Review wave sees (consistent with `## Recovery`'s reconcile-from-tree). For an XL cumulative diff, native `/code-review` (ultra thinking) is the documented alternative to the End wave.
...
**artifacts on disk, handles in context.** When the same large `<APPROVED_PLAN>` must reach many consumers across a run, paying its full byte cost into every fill is waste: the orchestrator writes the plan to disk ONCE and hands each consumer a short handle - a path to read - instead of the inline block. The handle is a read-imperative envelope: `Read the verbatim <APPROVED_PLAN> at <path> - its bytes ARE the artifact.` The bytes on disk ARE the verbatim artifact; the handle just defers the read.

**Handle consumers.** The handle reaches the enumerated five parallel End Review reviewers - `acceptance-reviewer`, `conventions-reviewer`, `correctness-reviewer`, `shape-reviewer`, `simplicity-reviewer` - PLUS five more `<APPROVED_PLAN>` consumers: `code-implementer`, `plan-challenger`, `fixer`, `test-plan`, `test-review`. These five carry the in-body handle-read line (after their `## Input` block), so when their slot holds a handle they Read the file; the handle source for them is the same `plan-<slug>.md` the reviewers read.

**Inline carve-out (two, each with its reason).** `plan-arbiter` stays inline because on a multi-plan run it reads N distinct competing plans, which one shared `plan-<slug>.md` cannot serve. `safety-gate` stays inline because it is the solo gate consumer on the system route, so the handle indirection buys nothing.

**plan-challenger is single-plan-only.** It takes the handle ONLY on a single-plan run, since there is one plan to share. On a multi-plan run the N challengers each critique a DIFFERENT competing plan, so the orchestrator inlines each - one shared handle cannot serve N distinct plans. Harmlessness: the in-body handle-read line activates only when the slot actually holds a handle, so it never misfires on an inlined consumer.

**Union gate (state it exactly).** Offload only when the plan block exceeds `RIVER_ARTIFACT_OFFLOAD_CHARS` (a fixed documented threshold, ~1500 chars - a constant of this doctrine, not a runtime-read knob) AND ( the parallel End Review wave carries more than one plan-consuming reviewer OR `#significant-build` is live ). Anchor the why: `correctness-reviewer` fires on every code build (`#code-written`) and `simplicity-reviewer` fires on every planned build (`#plan-ready`), so a `#clarified-but-not-significant` planned build with a >1500-char plan already forms a >1 plan-consuming wave and must still offload; the `#significant-build` term adds the sequential consumers (`code-implementer`, `test-plan`, `test-review`, `fixer`) that fire outside the parallel wave. A smaller plan stays inlined; both conditions must hold.

**Write timing.** The orchestrator DECIDES the offload and the `code-planner` PERFORMS the write. When the union gate fires the orchestrator - which owns the union-condition terms (the >1 plan-consuming wave / `#significant-build`) and computes `<slug>` as the kebab-case reduction of the plan's subject by the same rule the briefs use (`doctrine/briefs.md:32`) - hands the planner the target path `.alp-river/artifacts/plan-<slug>.md` in `<ARTIFACT_OFFLOAD>`; the code-planner writes the verbatim plan block there in the same spawn that produces it. The size threshold has ONE owner: the orchestrator owns the single `RIVER_ARTIFACT_OFFLOAD_CHARS` measurement and re-applies it to the returned block for the handle-vs-inline decision - the producer never re-measures, so the threshold is never duplicated across two owners. A plan that satisfies the union condition but comes back under threshold is still written to the path yet inlined to consumers - a negligible write, the price of single ownership. On a Revise or forward-only re-split the same path is supplied, so the code-planner overwrites the file in place - boundary-only, never in flight; absent a re-split nothing rewrites. The file is refreshed, not appended, and is not immutable - the latest write is the one every consumer reads.

**Milestone-loop timing.** Under the milestone loop the plan mutates mid-run, so the orchestrator overwrites `plan-<slug>.md` ONLY at a milestone BOUNDARY - before the next milestone's implementer run, NEVER in-flight - because an in-flight overwrite would swap the artifact under a consumer still reading it.

**input_template is UNCHANGED for the handle-vs-inline mechanism.** The handle rides at fill-time only. `generated/catalog.json` `input_template` does not regenerate on account of the handle: every consumer's template still declares the `<APPROVED_PLAN>` slot exactly as before, and the orchestrator chooses, per the union gate above, whether to fill that slot with the inline block or the handle envelope. This is separate from `code-planner`'s own `<ARTIFACT_OFFLOAD>` producer slot (Write timing, above), which IS a genuine `input_template` addition on `code-planner` - the one agent that performs the write. Outside that one addition, nothing regenerates, no template moves.
...
`hooks/recover-run-state.sh` is the single SessionStart recovery hook, registered on
startup, resume, and compact. It re-anchors the workflow pointer plus the canonical run
state: the current route, live signals, available artifacts, and premises. It reads
**file-first** from the per-run `.alp-river/runs/<run-id>/run-state.json` (`## Durability`).
When that file is absent or invalid it emits the workflow anchor plus a preserve-manually
note - the on-disk file is the sole state source; there is no transcript recovery. The
router recomputes the route from the re-anchored blocks, so resumption is deterministic.
Preserve manually only what is not in those blocks: the stage currently mid-run and any
gate awaiting the user. A pending plan-approval confirm (system or small planned build, waiting on
the orchestrator-emitted `#plan-approved`) is one such gate awaiting the user, preserved by
that same rule. A brief pull in flight (`doctrine/briefs.md`) is the same case folded in:
the gate never leaves the picker, so preserving the gate awaiting the user carries the
pending paste-back too.
