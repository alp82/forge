...
**Handle consumers.** The handle reaches the enumerated ten parallel End Review reviewers - `acceptance-reviewer`, `architecture-reviewer`, `assumptions`, `consistency-reviewer`, `correctness-reviewer`, `naming-clarity`, `quality-reviewer`, `reuse-reviewer`, `simplicity-reviewer`, `structure-reviewer` - PLUS five more `<APPROVED_PLAN>` consumers: `code-implementer`, `plan-challenger`, `fixer`, `test-plan`, `test-review`. These five carry the in-body handle-read line (after their `## Input` block), so when their slot holds a handle they Read the file; the handle source for them is the same `plan-<slug>.md` the reviewers read.

**Inline carve-out (three, each with its reason).** `plan-adherence-reviewer` stays inline because its `<APPROVED_PLAN>` MUST be the EXACT version pinned in the implementer's spawn, which an overwrite-in-place file would not preserve - so it always receives the verbatim inline block to guarantee version fidelity. `plan-arbiter` stays inline because on a multi-plan run it reads N distinct competing plans, which one shared `plan-<slug>.md` cannot serve. `safety-gate` stays inline because it is the solo gate consumer on the system route, so the handle indirection buys nothing.
...
**Milestone-loop timing.** Under the milestone loop the plan mutates mid-run, so the orchestrator overwrites `plan-<slug>.md` ONLY at a milestone BOUNDARY - before the next milestone's implementer run, NEVER in-flight - because an in-flight overwrite would swap the artifact under a consumer still reading it. `plan-adherence-reviewer`'s inline-pinned copy tracks the SAME per-milestone version the implementer built against, so the handle source and the adherence check never diverge.
