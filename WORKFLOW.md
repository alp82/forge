...
A hard session kill - not just compaction - is recoverable because the orchestrator persists its canonical run state to disk every turn. The snapshot lives at the per-run path `.alp-river/runs/<run-id>/run-state.json`, where `<run-id>` is the session id, so two pipelines in one repo never clobber one file. The orchestrator writes it as the HARD REQUIRED step-4 action through its own Write tool (loop step 4 owns the write contract and the exact schema; this section does not restate them). The SessionStart recovery hook `hooks/recover-run-state.sh` (registered on startup, resume, and compact) reads it back and always injects the per-run write path so the orchestrator learns where to persist.
...
**Write timing.** When the gate fires the orchestrator writes the plan once to `.alp-river/artifacts/plan-<slug>.md` at plan-creation - the orchestrator is the sole writer, the same orchestrator-writes-the-doc precedent the briefs follow (`doctrine/briefs.md:15`), and `<slug>` is the kebab-case reduction of the plan's subject by the same rule the briefs use (`doctrine/briefs.md:32`). On a Revise the orchestrator overwrites it in place with the new plan version; the file is refreshed, not appended, and is not immutable - the latest write is the one every consumer reads.
...
**input_template is UNCHANGED.** The handle rides at fill-time only. `generated/catalog.json` `input_template` does not regenerate - every consumer's template still declares the `<APPROVED_PLAN>` slot exactly as before; the orchestrator chooses, per the union gate above, whether to fill that slot with the inline block or the handle envelope. Nothing regenerates, no template moves.
