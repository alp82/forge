...
4. **Update.** Silently add the stage's `output` artifacts to `available`, its published `#signals` to `live` (each carries a one-line message = the why), and record it in `ran`. The two are different keys, never conflated: `triage`'s artifact `@confirmed-intent` lands in `available`, while its signal `#intent-confirmed` lands in `live`. A published `scope-shift` logs a broken premise. For a reviewer, the published signal is read from the explicit `SIGNALS_PUBLISHED:` token inside its output (see `doctrine/reviewer-contract.md` ### Published-signal line), not inferred from VERDICT prose. These updates are the state the next turn's step 1 recomposes from. **Then persist the run state** - after updating `available`/`live`/`ran`, the orchestrator DISPATCHES the `run-state-writer` subagent (an off-route utility with no stage block and no signals - the setup-agent precedent) to write the run-state snapshot to disk, a HARD REQUIRED step-4 action, as non-optional as the `#plan-approved` the orchestrator owes itself on the system and trivial-code paths. It hands the writer the injected `<WRITE_PATH>`, the `run_id`, the `cwd`, and the labeled state fields; the writer OWNS the Write and authors the JSON. The dispatch is fire-and-forget in the background (`Agent(run_in_background: true)`): the orchestrator keeps no handle and runs no watchdog over it, and it is silent and off-route so the loop stays **card-only** - no inline `<RUN_STATE>` block is ever emitted into the conversation, so no card-only carve-out is needed. Fire-and-forget is a weaker guarantee than the prior synchronous Write: two in-flight writers can land out of order, and an in-flight write can be lost if the session is hard-killed mid-write - but this DEGRADES rather than breaks, because the durability-scoping sentence below already rests durability on route-recomputation plus reconcile-from-tree, NOT on the snapshot being perfectly fresh. The target is the per-run path the SessionStart recovery hook injects into context: `<cwd>/.alp-river/runs/<run-id>/run-state.json`, where `<run-id>` is the session id; the orchestrator does not invent the path, it is told the path by the injected context. The snapshot is exactly `{schema_version, run_id, cwd, route, live, available, ran, premises, mid_run_stage, pending_gate, pending_gate_question, artifact_index}` with these types: `route`, `live`, `available`, and `ran` are JSON ARRAYS (of stage / signal / artifact names); `run_id`, `cwd`, `mid_run_stage`, `pending_gate`, and `pending_gate_question` are strings; `artifact_index` is an object; `schema_version` is a number. There is no `written_at` field, freshness is the FILE's mtime, set by the OS on each write. Keep the free-text fields (`premises`, `pending_gate_question`) terse and quote-light: the run-state-writer authors this JSON, so a stray unescaped quote or newline yields malformed JSON, and since the read path validates and silently falls back a malformed write costs durability invisibly - terseness is the mitigation. Durability rests on this write being a hard step-4 action AND on `already_run`/`live`/`available` accuracy - the router recomputes the route from them - NOT on the age guard catching a missed write: a missed write leaves a recent-but-stale file the age guard would pass, so the backstop is route-recomputation plus reconcile-from-tree, not freshness. This trades a zero-inference native Write for one background subagent spawn per loop turn - an accepted cost, paid deliberately to keep every `.alp-river` write off the orchestrator. The write surface stays hidden in subagents: the orchestrator NEVER writes or edits `run-state.json` inline - no Edit or Write on the file from the orchestrator, not even as a fallback after a failed write - because a visible inline write re-opens the exact surface this dispatch exists to close; on a failed or missing writer result it re-dispatches the `run-state-writer` (bounded - one re-dispatch, then the next turn's hard step-4 dispatch carries the state), fire-and-forget as ever.
...
gate awaiting the user. A pending plan-approval confirm (system or trivial-code, waiting on
