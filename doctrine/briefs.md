...
The trigger is an orchestrator-inline rule, NOT a signal: the user picked the `See it as an interactive doc` option at a picker, so the orchestrator writes the `.briefs/<touchpoint>-<slug>.html` doc and waits for the paste-back. This is handled exactly like the trivial-code Hold-escalates-to-challenge precedent in `WORKFLOW.md` ### Gates / ## Locks - escalation lives in the option, the orchestrator reads the pick inline. No catalog routing, no new signal.
...
c. **One consistent escalation label.** The option reads `See it as an interactive doc` across every touchpoint, so the user learns one affordance once.
