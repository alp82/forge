#!/usr/bin/env python3
"""AfterTool(edit tools) hook: arm the per-session "code changed" markers
(gemini port).
...
scratch dir (.forge/, or the pre-2.0 .alp-river/). The AfterAgent gates
...
Gemini I/O (RISK-4 - the survey documents only write_file / replace by name and
never pins their input fields, so this reads tolerantly):
  - Matched tool names: gemini's write_file / replace, plus the broad
    cross-harness set apply_patch / edit / write / str_replace / create_file and
    the Claude-shaped Edit / Write. This set mirrors the AfterTool matcher in
    hooks.json exactly (test_edit_tools_match_hooks_json_matcher pins the sync).
...
This hook only ARMS. The review debt is settled at AfterAgent by review-owed.py - see
...
and never prints (AfterTool stdout would be parsed). Fail-open on the outer
...
# RISK-4: only gemini's write_file / replace are documented edit tools; match
# broadly across every plausible cross-harness spelling. This set MUST stay in
# sync with the AfterTool matcher in hooks.json - the matcher decides whether
# gemini invokes the hook, this set decides whether the hook arms, and a name in
# one but not the other silently disarms the gated tier.
...
def main():
    try:
        ...
        # Rendezvous precondition: this session_id must equal the end-of-turn AfterAgent
        ...
        # subagent AfterTool firing under a different session_id arms the wrong
        # marker (RISK-8).
