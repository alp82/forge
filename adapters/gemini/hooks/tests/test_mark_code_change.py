"""Tests for hooks/mark-code-change.py - the gemini PostToolUse marker armer.
...
CONTRACT (gemini port - RISK-4 tolerant reads):
  ...
  - Arms three session-keyed marker files when any derived path is a real
    ...
    Markers: /tmp/.gemini-changed-tests-<session_id>
             /tmp/.gemini-changed-build-<session_id>
             /tmp/.gemini-changed-review-<session_id>
  ...
  - session_id missing/empty falls back to a pid-keyed marker name:
    /tmp/.gemini-changed-<kind>-fallback-<pid>
...
def _markers(session_id):
    return (
        Path(f"/tmp/.gemini-changed-tests-{session_id}"),
        Path(f"/tmp/.gemini-changed-build-{session_id}"),
        Path(f"/tmp/.gemini-changed-review-{session_id}"),
    )
...
# ---------------------------------------------------------------------------
# TC-MCC-GEMINI: gemini's own documented edit tools (write_file / replace) arm.
# Regression guard for the port bug where the internal EDIT_TOOLS set was copied
# verbatim from codex and omitted write_file/replace, so the hook fired (matcher
# in hooks.json) but silently no-op'd (internal set), disarming the gated tier.
# ---------------------------------------------------------------------------


def test_gemini_documented_edit_tools_arm():
    for tool in ("write_file", "replace"):
        ...
        try:
            ...
            assert tests_marker.exists(), (
                f"gemini edit tool {tool!r}: expected tests marker at {tests_marker}"
            )
            assert build_marker.exists(), f"gemini edit tool {tool!r}: build marker"
            assert review_marker.exists(), f"gemini edit tool {tool!r}: review marker"
        finally:
            shutil.rmtree(cwd, ignore_errors=True)
            _cleanup(tests_marker, build_marker, review_marker)


def test_edit_tools_match_hooks_json_matcher():
    """The internal EDIT_TOOLS set and the AfterTool matcher in hooks.json must
    stay in sync: a tool name that gemini routes to the hook (matcher) but the
    hook rejects (EDIT_TOOLS) silently disarms the gated tier, and vice versa.
    """
    source = MARK_CODE_CHANGE_PY.read_text()
    block = re.search(r"EDIT_TOOLS = frozenset\(\s*(\{.*?\})\s*\)", source, re.DOTALL)
    assert block, "could not locate the EDIT_TOOLS frozenset in mark-code-change.py"
    edit_tools = set(re.findall(r'"([^"]+)"', block.group(1)))

    config = json.loads(HOOKS_JSON.read_text())
    matcher = re.compile(config["hooks"]["AfterTool"][0]["matcher"])
    for tool in edit_tools:
        assert matcher.fullmatch(tool), (
            f"{tool!r} arms in EDIT_TOOLS but the AfterTool matcher never routes "
            f"it to the hook - it can never fire"
        )


# ---------------------------------------------------------------------------
# TC-MCC-02: lower-case gemini tool names arm identically
# ---------------------------------------------------------------------------


def test_lowercase_gemini_tool_names_arm_identically():
...
def test_missing_session_id_falls_back_to_pid_keyed_markers():
    before = (
        set(glob.glob("/tmp/.gemini-changed-tests-fallback-*"))
        | set(glob.glob("/tmp/.gemini-changed-build-fallback-*"))
        | set(glob.glob("/tmp/.gemini-changed-review-fallback-*"))
    )
    cwd = tempfile.mkdtemp()
    new_markers = []
    try:
