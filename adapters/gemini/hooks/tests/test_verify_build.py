...
/tmp/.gemini-changed-build-<session_id> is present (armed by
...
CHANGE_PREFIX = "gemini-changed-build"
...
def test_no_build_command_is_silent_pass():
    ...
    marker = Path(f"/tmp/.gemini-build-verify-{session_id}")
...
def test_failing_build_with_special_chars_emits_valid_json_block():
    ...
    try:
        ...
        assert result.returncode == 0, (
            f"hook must always exit 0 (gemini hook contract); "
...
def test_retry_cap_second_call_is_not_blocked():
    ...
    Flow:
      ...
      2. Marker file /tmp/.gemini-build-verify-<session_id> must now exist,
...
def test_vb_g01_change_marker_absent_failing_dir_is_silent_pass():
    ...
    session_id = str(uuid.uuid4())
    verify_marker = Path(f"/tmp/.gemini-build-verify-{session_id}")
    build_dir = _make_failing_build_dir()
    try:
        result = _run_hook(
            {"session_id": session_id, "cwd": build_dir, "stop_hook_active": False}
