...
Contract: mark-code-change.py arms /tmp/.gemini-changed-review-<sid> on
...
def _review_marker(session_id):
    return Path(f"/tmp/.gemini-changed-review-{session_id}")


def _retry_marker(session_id):
    return Path(f"/tmp/.gemini-review-owed-{session_id}")
