"""Tests for hooks/session-start.sh - the gemini BeforeAgent injection.
...
"re-run the setup-forge skill" nag. setup-forge writes ~/.gemini/agents/
forge-<tier>.md files stamped `<!-- forge-version: X.Y.Z -->`; a stamp differing
from the extension version, a missing stamp, or an incomplete tier set nags. No
forge agent file at all means setup was never run and stays silent.

Driven with HOME pointed at a temp dir so the real ~/.gemini never leaks into
assertions. The script self-locates via BASH_SOURCE (no extension-root env var
is guaranteed under a settings.json-wired hook - RISK-7). The version source is
the native channel manifest gemini-extension.json at the repo root.
"""
...
SESSION_START_SH = REPO_ROOT / "adapters" / "gemini" / "hooks" / "session-start.sh"
...
def _plugin_version():
    return json.loads(
        (REPO_ROOT / "gemini-extension.json").read_text()
    )["version"]
...
def _write_agents(home, stamp, tiers=TIERS):
    agents = Path(home) / ".gemini" / "agents"
    agents.mkdir(parents=True, exist_ok=True)
    for tier in tiers:
        body = f"---\nname: forge-{tier}\n---\n"
        if stamp is not None:
            body += f"<!-- forge-version: {stamp} -->\n"
        (agents / f"forge-{tier}.md").write_text(body)
...
def test_stale_agent_stamp_triggers_the_setup_forge_nag():
    ...
    try:
        ...
        assert "setup-forge" in ctx, (
            f"agents stamped 0.0.1 against extension {_plugin_version()} must nag: {ctx!r}"
        )
    finally:
        shutil.rmtree(home, ignore_errors=True)
...
def test_non_forge_agent_files_stay_silent():
    """A user's own agent files (not forge-<tier>.md) never trigger the nag."""
    home = tempfile.mkdtemp()
    try:
        agents = Path(home) / ".gemini" / "agents"
