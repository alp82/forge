"""Tests for hooks/block-git-writes.sh - the gemini PreToolUse shell guard.

CONTRACT (gemini denial channel - NOT the Claude stdout-JSON shape):
  ...
  - BLOCK: exit 2 with the reason written to stderr - the survey's documented
    gemini blocking mechanism ("Exit code 2: indicates blocking/denial with
...
def run_guard(command=None, tool_name="Bash", env=None):
    ...
    Builds the stdin JSON and returns the CompletedProcess (the gemini contract
...
# Gemini-specific I/O cases: the shell tool may appear under any of the matched
# names (RISK-2); Read stays outside the matcher set and is always allowed.
CASES += [
    ("TC-gemini-shell-block", "shell", "git reset --hard", True),
    ("TC-gemini-shell-allow", "shell", "git add -A", False),
    ("TC-gemini-local-shell-block", "local_shell", "git push --force", True),
    ("TC-gemini-bash-lower-block", "bash", "git rebase -i HEAD~3", True),
    ("TC-gemini-exec-command-block", "exec_command", "git clean -fd", True),
]
...
def test_block_git_writes(case_id, tool_name, command, expected_block):
    ...
    against the gemini denial channel (exit 2 + stderr reason)."""
...
def test_jq_present_parse_failure_stays_fail_closed():
    ...
    exit 2 with the parse-failure reason on stderr (the gemini denial channel).
