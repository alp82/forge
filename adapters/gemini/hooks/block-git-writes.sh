#!/usr/bin/env bash
# PreToolUse hook (gemini port): block the dangerous, allow the forward.
...
# passes. Logs blocked attempts to ~/.gemini/debug/git-block.log (rotated at ~1MB).
#
# Gemini denial channel: exit 2 with the reason on stderr - the one blocking
# mechanism the gemini survey documents verbatim ("Exit code 2: indicates
# blocking/denial with reason written to stderr"). Allow = exit 0, no output.

set -euo pipefail

log_file="$HOME/.gemini/debug/git-block.log"
mkdir -p "$HOME/.gemini/debug" 2>/dev/null || true
...
# goes to stderr, exit 2 (gemini denial channel).
block_static() {
  local reason="$1"
...
# Only evaluate shell tool calls. RISK-2: gemini's shell tool name is
# undocumented - match the plausible spellings broadly.
if [[ ! $tool_name =~ ^(run_shell_command|shell|local_shell|bash|Bash|exec_command)$ ]]; then
  exit 0
fi
