#!/usr/bin/env python3
"""AfterAgent gate: code changed this session, so a review wave is owed (gemini port).
...
every real project edit; the debt is settled here, at AfterAgent, when a lens
...
subagent session_id - so a AfterTool clearer neither fires nor rendezvous
with the main session's marker. This AfterAgent gate runs under the main session_id
...
re-arm it). Respects stop_hook_active so an already-blocked AfterAgent loop is never
re-blocked (RISK-5: gemini may never send it; the retry cap is the structural
loop-breaker either way).

Stdlib only. Always exits 0 (a block is a decision, not a hook error). The
block is a single {"decision":"block"} JSON object on stdout - gemini's
documented AfterAgent block channel; every pass/skip branch prints nothing.
"""
...
RETRY_PREFIX = "gemini-review-owed"
...
def main():
    ...
    # Inside an already-blocked AfterAgent loop: stand down for this turn, keep the
