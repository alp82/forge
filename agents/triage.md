...
- `est-size:<tier>` - one advisory shirt size (XS-XXL) read off the request's shape; the orchestrator reads it only for the small-planned-build auto-release (`WORKFLOW.md` ## Locks, Release policy). The real size stays the final route count.
