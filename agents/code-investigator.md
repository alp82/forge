...
5. **Minimal repro**: Execute the chosen trigger in the smallest possible surface. Use Bash to actually run it.
   - `SEVERITY: high | critical` - repro is REQUIRED, because a fix at these stakes must rest on demonstrated behavior, not speculation. If you cannot repro, report that clearly - do not speculate a cause.
